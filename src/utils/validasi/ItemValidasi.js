...
const find = async (nama) => await prompt(`CALL read_itemsNama("${nama}")`);

// cari "apakah id sudah digunakan sebagai isi items dalam tabel joins_items?"
const findRestrict = async (id) =>
  await prompt(`CALL read_joins_itemsItemsID("${id}")`);
...
exports.validDelete = async (req, res, next) => {
  ...
  if (restrict[0].length > 0)
    return res.status(400).json({
      message: "Hapus terlebih dahulu data pada joins_items!",
    });
